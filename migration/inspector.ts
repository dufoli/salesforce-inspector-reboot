let apiVersion = localStorage.getItem("apiVersion")?? "59.0";

interface RestOption {logErrors: boolean, method: string, api: string, body: any, bodyType: string, responseType: XMLHttpRequestResponseType, headers: any, progressHandler: any};
interface XMLStringifyParams {name: string, attributes: any, value : any};
...
let sfConn = {
  ...
  async getSession(sfHost: string) {
    ...
    let message: Response = await new Promise(resolve =>
      chrome.runtime.sendMessage<{message: string, sfHost: string}, Response>({message: "getSession", sfHost}, resolve));
    if (message) {
  ...
  async rest(url: string, option: RestOption = {logErrors : true, method : "GET", api : "normal", body : undefined, bodyType : "json", responseType : "json", headers : {}, progressHandler : null}) {
    ...
    xhr.open(option.method, "https://" + this.instanceHostname + url, true);

    xhr.setRequestHeader("Accept", "application/json; charset=UTF-8");

    if (option.api == "bulk") {
      xhr.setRequestHeader("X-SFDC-Session", this.sessionId);
    } else if (option.api == "normal") {
    ...
    for (let [name, value] of Object.entries(option.headers)) {
    ...
    if (xhr.status >= 200 && xhr.status < 300) {
      ...
      if (!option.logErrors) { console.error("Received no response from Salesforce REST API", xhr); }
      let err = new Error();
      err.name = "SalesforceRestError";
      err.message = "Network error, offline or timeout";
      throw err;
    } else {
      if (!option.logErrors) { console.error("Received error response from Salesforce REST API", xhr); }
      let err = new APIError("REST API error");
      err.name = "SalesforceRestError";
      err.detail = xhr.response;
      try {
        err.message = err.detail.map((err: { errorCode: any; message: any; fields: any[]; }) => `${err.errorCode}: ${err.message}${err.fields && err.fields.length > 0 ? ` [${err.fields.join(", ")}]` : ""}`).join("\n");
      } catch (ex) {
  ...
  wsdl(apiVersion: string, apiName: string) {
    let wsdl = {
      Enterprise: {
    ...
    if (apiName) {
      return wsdl[apiName as keyof typeof wsdl];
    }
    return wsdl;
  },

  async soap(wsdl: { servicePortAddress: string; apiName: string; targetNamespaces: any; }, method: string, args: any, {headers} = {headers:{}}) {
    if (!this.instanceHostname || !this.sessionId) {
      throw new Error("Session not found");
    }
    ...
    if (xhr.status == 200) {
      ...
      if (responseBody == null) {
        ...
        err.message = "response body is empty";
        throw err;
      } else {
      ...
      console.error("Received error response from Salesforce SOAP API", xhr);
      let err = new APIError("SOAP API error");
      err.name = "SalesforceSoapError";
      err.detail = xhr.response;
      try {
...
class XML {
  static stringify({name, attributes, value} : XMLStringifyParams) {
    function buildRequest(el: HTMLElement, params: {} | null) {
      if (params == null) {
  ...
  static parse(element: any) {
    function parseResponse(element: Element) {
...
let inspector = {sfConn, apiVersion};
