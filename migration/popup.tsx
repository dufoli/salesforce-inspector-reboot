...
import {sfConn, apiVersion} from "./inspector";
import {getAllFieldSetupLinks} from "./setup-links";
import {setupLinks} from "./links";

let h = React.createElement;

{
  window.parent.postMessage({insextInitRequest: true}, "*");
  window.addEventListener("message", function initResponseHandler(e) {
    if (e.source == this.window.parent && e.data.insextInitResponse) {
      window.removeEventListener("message", initResponseHandler);
      init(e.data);
      initLinks(e.data);
    }
  });
...
interface EntityOption {name: String, label: String, keyPrefix: any, durableId: any, isCustomSetting: any, recordTypesSupported: any, isEverCreatable: any, newUrl: any};

function closePopup() {
  window.parent.postMessage({insextClosePopup: true}, "*");
}

function showApiName() {
    window.parent.postMessage({insextShowApiName: true}, "*");
}

function init(p: {sfHost : string, inDevConsole: boolean, inLightning : boolean, inInspector: boolean}) {
  let addonVersion = chrome.runtime.getManifest().version;
  let {sfHost, inDevConsole, inLightning, inInspector} = p;
  sfConn.getSession(p.sfHost).then(() => {
    const root = document.createElement("div");
    root.className = "container";
    document.body.appendChild(root);
    const rootDiv = ReactDOM.createRoot(root);
    rootDiv.render(h(App, {
      sfHost,
      inDevConsole,
      inLightning,
      inInspector,
...
function initLinks({sfHost}: {sfHost: string}){
  //add custom links to setupLink
  if (localStorage.getItem(sfHost + "_orgLinks")){
    let val = window.localStorage.getItem(sfHost + "_orgLinks");
    if (val == null) {
        return;
    }
    let links = JSON.parse(val);
    links.forEach((link: { label: string; link: string; section: string; prod: boolean; }) => {
...
class App extends React.PureComponent {
  ...
  onContextUrlMessage(e: { source: Window; data: { insextUpdateRecordId?: any; isFieldsPresent?: any; locationHref?: any; }; }) {
    if (e.source == window.parent && e.data.insextUpdateRecordId) {
  ...
  componentDidMount() {
    ...
    window.parent.postMessage({insextLoaded: true}, "*");
...
class AllDataBox extends React.PureComponent<any,any> {
  SearchAspectTypes: Readonly<{ sobject: "sobject"; users: "users"; shortcuts: "shortcuts"; org: "org"; }>;
    parseContextUrl: () => void;
  ...
  componentDidUpdate(prevProps: { contextUrl: any; }, prevState: { activeSearchAspect: any; }) {
  ...
  setIsLoading(aspect: string, value: any) {
    switch (aspect) {
  ...
  onAspectClick(e: { currentTarget: { dataset: { aspect: any; }; }; }) {
    this.setState({
  ...
  loadSobjects() {
    ...
    function addEntity({name, label, keyPrefix, durableId, isCustomSetting, recordTypesSupported, isEverCreatable, newUrl} : EntityOption, api: null) {
    ...
    function getObjects(url: string, api: string) {
...
class ShowDetailsButton extends React.PureComponent {
  ...
  onDetailsClick() {
    ...
    Promise.all([
      ...
      window.parent.postMessage({insextShowStdPageDetails: true, insextData: res, insextAllFieldSetupLinks}, "*");
      closePopup();
    }).catch(error => {
...
function getRecordId(href: string) {
...
function getSobject(href: string) {
  let url = new URL(href);
...
function getSfPathFromUrl(href: string) {
  let url = new URL(href);
...
function sfLocaleKeyToCountryCode(localeKey: string) {
...
export {getRecordId}; // for unit tests
